# =============================================================================
# TACS_alg.R
# Threshold-Adjusted ACS (TACS) navigation algorithm for the discretized SOS problem.
#
# Penalty function (threshold alpha = 0.5 by default):
#   P_TACS(c, p, alpha) =
#     c + 1                              if p < alpha
#     c + (1-p)^(-k_alpha)              if p >= alpha
#   where k_alpha = log(z0 - c) / log(1 / (1 - alpha))
#   and z0 is the zero-disambiguation path length approximation.
# =============================================================================

source("helpers.R")

# Function: tacs_penalty
# Computes the TACS penalty for a single obstacle given its probability mark.
# -----------------------------------------------------------------------------
tacs_penalty <- function(c, p, alpha = 0.5, z0) {
  # INPUT:
  #   c     : disambiguation cost
  #   p     : sensor probability mark
  #   alpha : threshold parameter (default 0.5)
  #   z0    : zero-disambiguation path length
  # OUTPUT: scalar penalty value
  if (p < alpha) {
    c + 1
  } else {
    k_alpha <- log(z0 - c) / log(1 / (1 - alpha))
    c + (1 - p)^(-k_alpha)
  }
}

# Function: Update_graph_TACS
# Initializes edge weights on graph g by adding TACS penalty terms for each
# obstacle that intersects each edge.
# -----------------------------------------------------------------------------
Update_graph_TACS <- function(g, x, y, circle_info, r, alpha = 0.5) {
  # INPUT:
  ...
  for (i in 1:n) {
    ...
    penalty <- tacs_penalty(circle_info[i, 3], circle_info[i, 4], alpha)
...
# Function: TACS_Alg
# Main TACS navigation algorithm. Iteratively finds shortest paths under
# current edge weights, traverse to the first ambiguous obstacle edge,
# disambiguates, and updates the graph accordingly.
# -----------------------------------------------------------------------------
TACS_Alg <- function(obs_info, r, x, y, s, t, alpha = 0.5) {
  # INPUT:
  ...
  #   alpha    : threshold parameter (default 0.5)
  # OUTPUT: list with Optimal_found, Length_total, Cost_total,
  #          Optimal_path, Disambiguate_state
  
  vertice_list  <- Lattice_Vertices(x, y)
  G_original    <- Graph_Discretized(x, y)
  out_Ginfo     <- Update_graph_TACS(G_original, x, y, obs_info, r, alpha)
  ...
  while (!reach_t) {
    ...
    if (is.null(D_state)) {
      ...
      penalty    <- tacs_penalty(obs_info[obs_ind, 3], p, alpha)
