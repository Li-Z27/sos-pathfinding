# =============================================================================
# ACS_alg.R
# Additive Cost-Sensitive ACS(k) navigation algorithm for the discretized SOS problem.
#
# Penalty function:
#   P_ACS(c, p, k) = c + (1 - p)^(-k),   k >= 0
# =============================================================================

source("helpers.R")

# Function: Update_graph_ACS
# Initializes edge weights on graph g by adding ACS(k) penalty terms for 
# each obstacle that intersects each edge.
# -----------------------------------------------------------------------------
Update_graph_ACS <- function(g, x, y, circle_info, r, acs_k) {
  # INPUT:
  #   g           : igraph object (base lattice graph)
  #   x, y        : grid dimension parameters
  #   circle_info : data frame with columns x, y, cost, prob, status
  #   r           : obstacle radius
  #   acs_k       : penalty exponent k
  # OUTPUT:
  ...
  for (i in 1:n) {
    ...
    n1 <- nrow(el)
    for (k in 1:n1){
      el[k, ] <- sort(el[k, ], decreasing = FALSE)
    }
    
    p <- circle_info[i, 4]
    # ACS(k) penalty: c + (1-p)^(-k)
    penalty <- circle_info[i, 3] + (1 - p)^(-acs_k)
...
ACS_Alg <- function(obs_info, r, x, y, s, t, acs_k = 1) {
  ...
  out_Ginfo     <- Update_graph_ACS(G_original, x, y, obs_info, r, acs_k)
  ...
  while (!reach_t) {
    ...
    if (is.null(D_state)) {
      ...
      penalty    <- obs_info[obs_ind, 3] + (1 - p)^(-acs_k)
