# =============================================================================
# RD_alg.R
# Reset Disambiguation (RD) navigation algorithm for the discretized SOS problem.
#
# Penalty function:
#   P_RD(c, p) = c / (1 - p)
# =============================================================================
...
# Function: Update_graph_RD
# Initializes edge weights on graph g by adding RD penalty terms for each
# obstacle that intersects each edge.
# -----------------------------------------------------------------------------
Update_graph_RD <- function(g, x, y, circle_info, r) {
  # INPUT:
  ...
  for (i in 1:n) {
    ...
    for (j in 1:n1) {
      ...
      # RD penalty: c / (1 - p); treated as Inf when p = 1
      if (circle_info[i, 4] == 1) {
        elg[idx, 3] <- Inf
      } else {
        elg[idx, 3] <- elg[idx, 3] + 0.5 * circle_info[i, 3] / (1 - circle_info[i, 4])
      }
      int_info[idx, i] <- 1
    }
  }
...
RD_Alg <- function(obs_info, r, x, y, s, t) {
  ...
  out_Ginfo     <- Update_graph_RD(G_original, x, y, obs_info, r)
  ...
  while (!reach_t) {
    ...
    if (is.null(D_state)) {
      ...
      penalty    <- obs_info[obs_ind, 3] / (1 - p)
