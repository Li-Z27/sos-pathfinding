# =============================================================================
# AP_alg.R
# Additive Penalty (AP) navigation algorithm for the discretized SOS problem.
#
# Penalty function:
#   P_AP(c, p) = c + (5p) / (1 - p^(1-p))
# =============================================================================

source("helpers.R")

# Function: Update_graph_AP
# Initializes edge weights on graph g by adding AP penalty terms for each
# obstacle that intersects each edge.
# -----------------------------------------------------------------------------
Update_graph_AP <- function(g, x, y, circle_info, r) {
  # INPUT:
  ...
  for (i in 1:n) {
    ...
    p <- circle_info[i, 4]
    # AP penalty: c + 5p / (1 - p^(1-p))
    penalty <- circle_info[i, 3] + (5 * p) / (1 - p^(1 - p))
...
# Function: AP_Alg
# Main AP navigation algorithm. Iteratively finds shortest paths under current
# edge weights, advances to the first ambiguous obstacle edge, disambiguates,
# and updates the graph accordingly.
# -----------------------------------------------------------------------------
AP_Alg <- function(obs_info, r, x, y, s, t) {
  # INPUT:
  ...
  out_Ginfo     <- Update_graph_AP(G_original, x, y, obs_info, r)
  ...
  while (!reach_t) {
    ...
    if (is.null(D_state)) {
      ...
      penalty    <- obs_info[obs_ind, 3] + (5 * p) / (1 - p^(1 - p))
